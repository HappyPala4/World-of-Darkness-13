...
/datum/preferences/proc/load_character(slot)
	...
	READ_FILE(S["selfcontrol"], selfcontrol)
	...
	READ_FILE(S["a_stamina"], Stamina)

	READ_FILE(S["a_charisma"], Charisma)
	READ_FILE(S["a_manipulation"], Manipulation)
	READ_FILE(S["a_appearance"], Appearance)

	READ_FILE(S["a_perception"], Perception)
	READ_FILE(S["a_intelligence"], Intelligence)
	READ_FILE(S["a_wits"], Wits)

	READ_FILE(S["s_alertness"], Alertness)
	READ_FILE(S["s_athletics"], Athletics)
	...
	READ_FILE(S["s_medicine"], Medicine)
	...
	READ_FILE(S["clane_sprite"], clane_sprite)
...
//		clane = new newtype()
	...
	consience				= sanitize_integer(consience, 1, 5, initial(consience))
	selfcontrol				= sanitize_integer(selfcontrol, 1, 5, initial(selfcontrol))
	courage				= sanitize_integer(courage, 0, 10, initial(courage))
	...
	Empathy				= sanitize_integer(Empathy, 0, 5, initial(Empathy))
	Intimidation				= sanitize_integer(Intimidation, 0, 5, initial(Intimidation))
	Expression				= sanitize_integer(Expression, 0, 5, initial(Expression))
	Crafts				= sanitize_integer(Crafts, 0, 5, initial(Crafts))
	Melee				= sanitize_integer(Melee, 0, 5, initial(Melee))
	Firearms				= sanitize_integer(Firearms, 0, 5, initial(Firearms))
	Drive				= sanitize_integer(Drive, 0, 5, initial(Drive))
	Security				= sanitize_integer(Security, 0, 5, initial(Security))
	Fleshcraft				= sanitize_integer(Fleshcraft, 0, 5, initial(Fleshcraft))
	Finance				= sanitize_integer(Finance, 0, 5, initial(Finance))
	Investigation				= sanitize_integer(Investigation, 0, 5, initial(Investigation))
	...
	clane_sprite = sanitize_inlist(clane_sprite, clane.alternative_sprites, null)
...
/datum/preferences/proc/save_character()
	...
	WRITE_FILE(S["selfcontrol"]			, selfcontrol)
	...
	WRITE_FILE(S["a_stamina"], Stamina)

	WRITE_FILE(S["a_charisma"], Charisma)
	WRITE_FILE(S["a_manipulation"], Manipulation)
	WRITE_FILE(S["a_appearance"], Appearance)

	WRITE_FILE(S["a_perception"], Perception)
	WRITE_FILE(S["a_intelligence"], Intelligence)
	WRITE_FILE(S["a_wits"], Wits)

	WRITE_FILE(S["s_alertness"], Alertness)
	WRITE_FILE(S["s_athletics"], Athletics)
	...
	WRITE_FILE(S["clane_sprite"]			, clane_sprite)
